-- Based on iDvel/rime-ice lua/autocap_filter.lua (GPL-3.0).
-- Preserve the original candidate so Rime can learn capitalization selections.
--[[
    ...
    通过 ShadowCandidate 保留原始候选，让大写输入也能学习词频
--]]
local function autocap_filter(input, env)
    ...
    for cand in input:iter() do
        ...
        elseif codeAllUCase then
            text = text:upper()
            yield(ShadowCandidate(cand, cand.type, text, cand.comment))
        -- 输入码首位大写，候选词转换为首位大写
