...
Installation finding (2026-09-12, macOS 26.6.2): the development bundle was copied into the user's Input Methods directory, its independent dictionary/configuration deployed, and the process launched successfully. However, TIS registration only exposed temporary entries; enabling returned success without enabling the parent input method, selection returned `-50`, and System Settings did not list the development source. `codesign --verify` passed while `spctl --assess --type execute` rejected the ad-hoc bundle. No signing identities were available on this machine. These are distinct checks: successful compilation/signature integrity does not establish system acceptance. Activation is still blocked. A [separate IMK project's macOS 26 investigation](https://github.com/ptrinh/viettelex/blob/main/docs/MACOS_IME_NOTES.md) reports a notarization requirement; this is a working hypothesis here, not a diagnosis confirmed by Apple's logs.

Follow-up checks confirmed that the registering process belongs to the correct user's Aqua session. Explicit Launch Services registration and a three-component bundle-version trial did not resolve selection; the installed bundle was restored from the verified build after that trial. The [upstream installer's completion screen](../../package/en.lproj/conclusion.html) explicitly requires saving work, logging out, and logging back in before adding the source in System Settings. That step has not yet been performed and is the next check; it is not guaranteed to resolve this case. Do not infer that buying a developer account is required solely from an ad-hoc bundle's failed Gatekeeper assessment. Investigate signing/notarization if activation still fails after completing the documented installation steps.
