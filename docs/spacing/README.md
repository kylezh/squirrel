...
Output: `build/spacing/SquirrelSpacingDev.app`, ad-hoc signed for this machine. The script does **not** install, enable, or switch input sources. The app has a separate bundle ID/IMK connection, uses `~/Library/RimeSpacingDev`, and disables the official Sparkle updater. It can coexist with the original input method. Command-line input-source registration helpers inherited from upstream still name the official IDs; use macOS Input Sources settings when trying this development identity.
