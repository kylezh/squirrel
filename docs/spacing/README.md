...
The user dictionary and learning database were copied to the independent development directory from a paused-process snapshot; original configuration files were verified unchanged. TextEdit has verified spacing enabled; other apps remain off. Desktop automation produced raw Latin letters under both the official and development input sources, so that control cannot certify IMK composition or spacing. Actual keyboard typing and the wider application matrix still need manual verification. Successful system registration/selection and passing in-process tests are recorded separately from that remaining work.
